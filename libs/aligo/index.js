...
const send = (obj, auth) => {
  // 문자보내기
  return formParse(obj, auth, "https://apis.aligo.in/send/").then(postRequest).catch(onError);
};

const sendMass = (obj, auth) => {
  // 문자보내기 대량
  return formParse(obj, auth, "https://apis.aligo.in/send_mass/").then(postRequest).catch(onError);
};

const list = (obj, auth) => {
  // 문자전송결과보기
  return formParse(obj, auth, "https://apis.aligo.in/list/").then(postRequest).catch(onError);
};

const smsList = (obj, auth) => {
  // 문자전송결과보기 상세
  return formParse(obj, auth, "https://apis.aligo.in/sms_list/").then(postRequest).catch(onError);
};

const remain = (obj, auth) => {
  // 문자발송가능건수
  return formParse(obj, auth, "https://apis.aligo.in/remain/").then(postRequest).catch(onError);
};

const cancel = (obj, auth) => {
  // 문자예약취소
  return formParse(obj, auth, "https://apis.aligo.in/cancel/").then(postRequest).catch(onError);
};
